...
// Copyright (c) 2013 Lvyecoin developers and contributors
...
//    Litecoin and modified for Lvyecoin.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated Lvyecoin network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for Lvyecoin
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the Lvyecoin network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Lvyecoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Lvyecoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Lvyecoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Lvyecoin is downloading blocks...");
